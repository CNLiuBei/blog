...
```
你的电脑 → Git Push → GitHub → Cloudflare Pages 自动检测 → 自动构建 → 自动部署
```

**完全自动化流程：**

1. **自动检测** - Cloudflare Pages 监听 GitHub 仓库的 Push 事件
2. **自动拉取** - 获取最新代码
3. **自动安装** - 执行 `npm install` 安装依赖
4. **自动构建** - 执行 `npm run build` 生成静态文件
5. **自动部署** - 发布到全球 CDN 网络
...
**生产环境：**
```bash
# main 分支 → https://your-blog.pages.dev
git checkout main
...
**预览环境：**
```bash
# 其他分支 → https://branch-name.your-blog.pages.dev
git checkout -b develop
git push origin develop
# 访问: https://develop.your-blog.pages.dev
```
...
# 访问预览: https://feature-new-theme.your-blog.pages.dev
